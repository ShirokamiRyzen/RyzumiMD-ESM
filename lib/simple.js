...
export function makeWASocket(connectionOptions, options = {}) {
    ...
    let sock = Object.defineProperties(conn, {
        ...
        getJid: {
            async value(sender, groupId) {
                try {
                    if (typeof sender !== 'string') return sender
                    // Already a normal JID
                    if (!sender.endsWith('@lid') || !groupId?.endsWith('@g.us')) return sender
                    const cached = (conn.chats[groupId] || {}).metadata
                    const metadata = cached || await conn.groupMetadata(groupId).catch(_ => null)
                    const participant = metadata?.participants?.find(p => (p.id === sender) || (p.jid === sender))
                    return participant?.jid || sender
                } catch (e) {
                    return sender
                }
            }
     ...
     * @param {import('@whiskeysocket/baileys').proto.WebMessageInfo} quoted - Message to reply to (optional)
     * @param {Boolean} ptt - If the file is a voice note
     * @param {Object} options - Additional options for sending media
     * @returns {Promise<import('@whiskeysocket/baileys').proto.WebMessageInfo>}
     */
     ...
     * @param {import('@whiskeysocket/baileys').proto.WebMessageInfo} quoted - The message to reply to (optional)
...
/**
    ...
    */
     ...
     * @param {import('@whiskeysocket/baileys').proto.WebMessageInfo} message - The message containing media
